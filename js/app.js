...
window.installPWA  = async () => {
  ...
  if (outcome === 'accepted') { $('#pwa-banner')?.classList.remove('show'); App.pwaPrompt = null; }
};
...
function runSplash() {
  return new Promise(res => {
    ...
    setTimeout(() => {
      ...
      el.style.opacity = '0';
      setTimeout(() => { el.classList.add('hidden'); el.style.opacity = ''; res(); }, 650);
    }, 2000);
  });
}
...
function storeAuth(email, pw) {
  const k = 'raksha_xor';
  const obf = (s) => btoa(s.split('').map((c,i) => String.fromCharCode(c.charCodeAt(0) ^ k.charCodeAt(i % k.length))).join(''));
...
function getStoredAuth() {
  try {
    ...
    const k = 'raksha_xor';
    const dob = (s) => atob(s).split('').map((c,i) => String.fromCharCode(c.charCodeAt(0) ^ k.charCodeAt(i % k.length))).join('');
    return { email: dob(e), password: dob(p) };
  } catch { return null; }
}
function clearStoredAuth() { localStorage.removeItem('raksha_auth'); }
...
window.obNext = () => {
  const total = $$('.ob-slide').length;
  if (obSlide < total - 1) { obSlide++; updateObSlide(); }
  else { localStorage.setItem('raksha_onboarded', '1'); stopObRenderers(); showAuthScreen('signup'); }
};
window.obSkip = () => { localStorage.setItem('raksha_onboarded', '1'); stopObRenderers(); showAuthScreen('signup'); };
...
window.doSignUp = async () => {
  ...
  let ok = true;
  if (!name)  { fieldErr('e-name',  'Name is required');           ok = false; } else fieldErr('e-name',  '');
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
              { fieldErr('e-email', 'Valid email required');        ok = false; } else fieldErr('e-email', '');
  if (!pw || pw.length < 6)
              { fieldErr('e-pw',    'Minimum 6 characters');        ok = false; } else fieldErr('e-pw',    '');
  if (pw !== pw2)
              { fieldErr('e-pw2',   'Passwords do not match');      ok = false; } else fieldErr('e-pw2',   '');
  if (!ok) return;
...
window.doSignIn = async () => {
  ...
  const pw    = document.getElementById('f-pw')?.value;
  let ok = true;
  if (!email) { fieldErr('e-email', 'Email required'); ok = false; } else fieldErr('e-email','');
  if (!pw)    { fieldErr('e-pw',    'Password required'); ok = false; } else fieldErr('e-pw','');
  if (!ok) return;
...
function card_btn() { return $('#auth-card .btn-primary'); }
...
function stopDashRenderers() {
  ['def-demo','atk-demo'].forEach(k => { App.demoRenderers[k]?.stop(); delete App.demoRenderers[k]; });
}
...
async function startTrainingScreen() {
  ...
  if (!App.trainerReady) {
    ...
    App.trainerReady = ok;
    if (!ok) { showToast('Pose detection unavailable', 'error'); return; }
  }
...
function handleFrame({ accuracy, feedbackMessages, accuracies }) {
  ...
  if (pose) {
    ...
    if (!prev?.completed && accuracy > (prev?.best_accuracy || 0)) {
      ...
      App._saveDebounce = setTimeout(() =>
        saveProgress(App.user?.id, pose.id, accuracy, false, null), 2500);
    }
  }
}
...
function hideHoldBar() { $('#hold-bar')?.classList.remove('show'); }
...
function showSummary(accuracy) {
  ...
  const logRows = App.feedbackLog.length > 0
    ? App.feedbackLog.map((entry, i) => {
  ...
  overlay.querySelector('.sum-acc').textContent =
    `Held with ${Math.round(accuracy)}% accuracy`;
...
// ── NAV helpers ───────────────────────────────────────────────
window.navHome    = () => { stopDashRenderers(); showDashboard(); };
window.navTrain   = () => { if (App.mode) showPoseTutorial(); else showDashboard(); };
