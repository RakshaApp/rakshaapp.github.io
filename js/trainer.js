...
class PoseTrainer {
  ...
  _onResults(results) {
    ...
    const w = this.canvas.width  = this.canvas.offsetWidth  || this.video.videoWidth  || 640;
    const h = this.canvas.height = this.canvas.offsetHeight || this.video.videoHeight || 480;
    ...
    // Draw ghost target skeleton
    drawTargetSkeleton(ctx, targets, w, h);

    // Draw user skeleton colored by accuracy
    drawOverlayHuman(ctx, lms, w, h, accuracies);
  ...
  _buildFeedback(lms, targets) {
    ...
    const seen = new Set();
    return msgs.filter(m => { if (seen.has(m.msg)) return false; seen.add(m.msg); return true; })
               .slice(0, 4);
  }
}
